                        ...
                        <table  class="datatable table display responsive nowrap">
                            ...
                            @foreach ($workshos as $key => $val)
                                <tr>
                                    ...
                                    <td>{{$val->getName($val->workshop_id)!=null?$val->getName($val->workshop_id)->name:''}}</td>
                                    <td>{{$val->total}}</td>
                                    <td style="padding-left:1rem">
                                        <a title="عرض الزوار" class="btn btn-primary btn-sm" style="margin:.1rem" href="{{ url('admin/BrowseWorkShopAttenders/'.$val->workshop_id) }}">
                                            <i class="fa fa-eye"></i>
                                        </a>
                                        <a title="تصدير إلى إكسل" class="btn btn-success btn-sm" style="margin:.1rem" href="{{ url('export-workshop/'.$val->workshop_id) }}">
